...
const RBG_KEYS = [
    'VwXQes36L5fpTjmMiFpwsy3W',
    'mkxdVteyNZZhx7fb6y6yqQ6o'
];
...
• .apsil https://resim  → Arkaplan olarak URL'deki fotoğrafı kullan`
    ...
    if (!message.reply_message?.image && !message.reply_message?.document) {
        return await message.send("❗ _Bir fotoğrafa veya belgeye yanıtlayarak yazınız._\n💬 Örnek: `.apsil`\n`.apsil kırmızı`\n`.apsil #00ff00`\n`.apsil https://...`");
    }

    const colorMap = {
        'kırmızı': 'ff0000', 'red': 'ff0000', 'mavi': '0000ff', 'blue': '0000ff', 'yeşil': '00ff00', 'green': '00ff00',
        'sarı': 'ffff00', 'yellow': 'ffff00', 'mor': '800080', 'purple': '800080', 'pembe': 'ff69b4', 'pink': 'ff69b4',
        'turuncu': 'ffa500', 'orange': 'ffa500', 'siyah': '000000', 'black': '000000', 'beyaz': 'ffffff', 'white': 'ffffff',
        'gri': '808080', 'gray': '808080', 'grey': '808080'
    };

    let userInput = '';
    if (typeof match === 'string') userInput = match.trim();
    else if (Array.isArray(match) && match[1]) userInput = match[1].trim();

    let bgColor = null, bgImageUrl = null;
    let processingMsg, okMsg;
    ...
    if (!userInput) {
    ...
    else if (userInput.toLowerCase().startsWith('http')) {
        ...
        return await message.send("❗ _Sadece renk yazabilir, hex kodu gönderebilir ya da resim URL'i belirtebilirsiniz._");
    }
    ...
    if (!response) {
        console.error('Tüm API anahtarları başarısız oldu:', lastError?.response?.data || lastError?.message);

        let errorMessage = '❌ _İşlem başarısız oldu!_';

        if (lastError?.response?.status === 400) {
            errorMessage = '❌ _Geçersiz parametre veya fotoğraf! Lütfen düz renk, hex kodu veya geçerli bir görsel URL girin._';
    ...
    try {
        ...
        const outputPath = `rbg_${Date.now()}.${extension}`;
