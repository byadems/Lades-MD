...
Module({
    ...
    const tytDate = moment("2026-06-20 10:15:00", "YYYY-MM-DD HH:mm:ss");
    const aytDate = moment("2026-06-21 10:15:00", "YYYY-MM-DD HH:mm:ss");
    const ydtDate = moment("2026-06-21 15:45:00", "YYYY-MM-DD HH:mm:ss");
    ...
    if (now.isAfter(moment(tercihbitis, "YYYY-MM-DD HH:mm:ss"))) {
        ...
        await m.sendReply(
            ...
            `⏳ *YDT* sınavına *${time3.days} gün ${time3.hours} saat ${time3.minutes} saniye* kaldı!\n📅 *21 Haziran 2026 - 15:45*`);
    }
    else if (now.isBefore(moment(sinavsonuc, "YYYY-MM-DD HH:mm:ss"))) {
        const timeToResults = calculateTime(sinavsonuc);
        await m.sendReply(`👀 YKS sonuçlarının açıklanmasına *${timeToResults.days} gün ${timeToResults.hours} saat ${timeToResults.minutes} dakika ${timeToResults.seconds} saniye* kaldı!\n📅 *22 Temmuz 2025 - 07:30*`);
    }
    else if (now.isBefore(moment(tercihbaslangic, "YYYY-MM-DD HH:mm:ss"))) {
        const timeToPreferences = calculateTime(tercihbaslangic);
        await m.sendReply(`🎓 YKS tercihlerinin başlamasına *${timeToPreferences.days} gün ${timeToPreferences.hours} saat ${timeToPreferences.minutes} dakika ${timeToPreferences.seconds} saniye* kaldı!\n📅 *31 Temmuz 2025*`);
    }
    else if (now.isBefore(moment(tercihbitis, "YYYY-MM-DD HH:mm:ss"))) {
        const timeToEnd = calculateTime(tercihbitis);
        await m.sendReply(`⏰ YKS tercihlerinin bitmesine *${timeToEnd.days} gün ${timeToEnd.hours} saat ${timeToEnd.minutes} dakika ${timeToEnd.seconds} saniye* kaldı!\n📅 *8 Ağustos 2025 - 23:59*`);
    }
    else {
        const time1 = calculateTime("2026-06-20 10:15:00");
        await m.sendReply(`🆕 2026 YKS süreci başladı!\n⏳ *TYT* sınavına ${time1.days} gün kaldı.\n📅 *20 Haziran 2026 - 10:15*`);
    }
});
...
Module({
    ...
    if (now.isAfter(targetDate)) {
        await m.sendReply(`❗ *OPS! MSÜ sınavı bu yıl için tamamlandı.* ✅\n📅 *1 Mart 2026 - 10:15*`);
    } else {
        const time = calculateTime(targetDate);
        await m.sendReply(`⏳ *MSÜ* sınavına *${time.days} gün ${time.hours} saat ${time.minutes} dakika ${time.seconds}* saniye kaldı!\n📅 *1 Mart 2026 - 10:15*`);
    }
});
...
Module({
    ...
    await m.sendReply(`🧐 En yakın tarih: *${closestDateObj.label}*\n⏳ ${closestDateObj.label === "Okulların açılışı" 
        ? "Okulların açılmasına" 
        : "Okulların kapanmasına"} *${time.days} gün ${time.hours} saat ${time.minutes} dakika ${time.seconds}* saniye kaldı! 🥳\n📅 *${formattedDate}*`);
});
...
Module({
    ...
    if (now.isBetween(moment(ramazanStart), moment(ramazanEnd))) {
        ...
        await m.sendReply(`⏳ Ramazan ayının bitmesine *${time.days} gün ${time.hours} saat ${time.minutes} dakika ${time.seconds} saniye* kaldı! 🥲\n📅 *19 Mart 2026 - Perşembe*`);
    } else if (now.isBefore(moment(ramazanStart))) {
        const time = calculateTime(ramazanStart);
        await m.sendReply(`⏳ Ramazan ayına girmemize *${time.days} gün ${time.hours} saat ${time.minutes} dakika ${time.seconds} saniye* kaldı! 😍\n📅 *19 Şubat 2026 - Perşembe*`);
    }
});
