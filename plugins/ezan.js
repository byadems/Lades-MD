...
async function getPrayerTimes(cityInput, type = 'ezan', date = moment().format('DD.MM.YYYY')) {
  ...
  try {
    ...
    if (!vakit) {
      return { error: '⚠️ Vakit bilgisi bulunamadı! Lütfen daha sonra tekrar deneyiniz.' };
    }
    ...
    if (type === 'ezan') {
      ...
      data = { Aksam: vakit.Aksam, HicriTarihUzun: vakit.HicriTarihUzun };
    } else if (type === 'sahur') {
      data = { Imsak: vakit.Imsak, HicriTarihUzun: vakit.HicriTarihUzun };
    }
    ...
    console.error(error);
    return { error: '⚠️ Ops! Bir hata oluştu. Lütfen daha sonra tekrar deneyiniz.' };
  }
}
