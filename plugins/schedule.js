...
function parseTime(timeStr) {
  ...
  const durationMatch =
    ...
    timeStr.match(/^(\d+)\s*(saat|s)\s*(\d+)\s*(dk|dakika)$/i) ||
    timeStr.match(/^(\d+)\s*(dk|dakika)\s*(\d+)\s*(sn|saniye)$/i);

  if (durationMatch) {
    let duration = moment.duration();

    if ((timeStr.includes("saat") || /\d+s\d+/i.test(timeStr)) && (timeStr.includes("dk") || timeStr.includes("dakika"))) {
  ...
  if (dateTime.isValid()) return dateTime.subtract(1, "minute").toDate();
...
Module(
  ...
  async (m, match) => {
    ...
    if (!match[1]) {
      return await m.sendReply("⚠️ _Lütfen üye etiketleyip zaman belirtiniz._");
    }
    ...
    const targetJid = m.mention?.[0];
    if (!targetJid) {
      return await m.sendReply("⚠️ _Lütfen bir üyeyi etiketleyin!_");
    }

    const timeStr = input.replace(/@\d+/g, "").trim();
    if (!timeStr) {
      return await m.sendReply("⚠️ _Lütfen zaman belirtin!_");
    }

    const scheduleTime = parseTime(timeStr);
    if (!scheduleTime) {
      return await m.sendReply(
        "❌ _Geçersiz zaman formatı_\n\n*⏱️ Desteklenen formatlar:*\n• `2 saat 30 dakika`, `2s30dk`\n• `1 gün`, `1g`\n• `30 dakika`, `30dk`\n• `5 saniye`, `5sn`\n• `14:30`, `14.30`\n• `25.12.2026 11:00`"
      );
    }
...
Module(
  ...
  async (m) => {
    try {
      ...
      if (pending.length === 0) return await m.sendReply("📭 _Bekleyen planlı mesaj bulunmuyor._");
...
Module(
  ...
  async (m, match) => {
    ...
    if (isNaN(messageId)) return await m.sendReply("⚠️ _Lütfen geçerli bir mesaj ID'si girin!_");

    try {
      const success = await scheduledMessages.delete(messageId);
      if (success) {
        await m.sendReply(`✅ *Planlı mesaj başarıyla silindi!*\n\n🗑️ *Mesaj ID:* ${messageId}`);
      } else {
